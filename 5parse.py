...
with open('params2.csv', 'rb') as csvfile:
	...
	for count in range(0, len(inputsort)-1):
		...
		if block2[2] == block1[2]:			# count consecutive found diskblocks 
			...
			if block2[1] == newblockdest:				# look for a new complete stripe line marker
				...
				storeddestblockinfo.append(newblockdest)	# parse the offset of the stripe line				
...
print "It seems that the blocksize used is: ",raidblocksize/2, " Kb. This is ",raidblocksize," 515-byte blocks" 
...
for count in range(0, len(storedblockinfo)):
	...
	if len(storedinputinfo[count])==disks-1:				# only print complete stripe lines, discard any incompletes
		print storedblockinfo[count],storeddestblockinfo[count]
	#print storedinputinfo[count], storedblockinfo[count],storeddestblockinfo[count]
